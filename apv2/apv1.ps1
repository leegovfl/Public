...
if (Test-Path -Path "$($env:ProgramData)\LeeCounty\PreProvision\PreProvisionV1.tag") {    
    $title    = 'Re-run Script'
...
if($hottogo){
    ...
    #{
        ...
        if ($decision -eq 0 -or $P) {
            ...
            Set-Content -Path "$($env:ProgramData)\LeeCounty\PreProvision\PreProvisionV1.tag" -Value "Installed"
